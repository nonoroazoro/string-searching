    ...
    else
    {
        ...
        while (i <= lengthT - lengthP)
        {
            for (let j = lengthP - 1; j >= 0; j--)
            {
                ...
                {
                    // 坏字符。
                    badShift = j - _getBadIndex(currentCharT, badCharacters);
                    if (goodSuffix !== "")
                    {
...
/**
 * 计算“好后缀规则表”。
 * 方法：拿到当前所有“好后缀”，从最长的开始，搜索其在 Pattern 中的“上一次出现位置”；如果未找到，则选短一点的，以此类推，直到最短的好后缀；如果都未找到，
 */
function _getGoodSuffixShifts(p_pattern)
{
}